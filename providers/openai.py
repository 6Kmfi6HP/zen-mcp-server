...
class OpenAIModelProvider(RegistryBackedProviderMixin, OpenAICompatibleProvider):
    ...
    def get_preferred_model(self, category: "ToolModelCategory", allowed_models: list[str]) -> Optional[str]:
        ...
        if category == ToolModelCategory.EXTENDED_REASONING:
            ...
            preferred = find_first(["gpt-5-codex-high", "gpt-5-pro", "o3", "o3-pro", "gpt-5-high"])
            return preferred if preferred else allowed_models[0]

        elif category == ToolModelCategory.FAST_RESPONSE:
            # Prefer fast, cost-efficient models
            # GPT-5 models for speed, GPT-5-Codex after (premium pricing but cached)
            preferred = find_first(["gpt-5-high", "gpt-5-mini", "gpt-5-codex-high", "o4-mini", "o3-mini"])
            return preferred if preferred else allowed_models[0]

        else:  # BALANCED or default
            # Prefer balanced performance/cost models
            # Include GPT-5-Codex for coding workflows
            preferred = find_first(["gpt-5-high", "gpt-5-codex-high", "gpt-5-pro", "gpt-5-mini", "o4-mini", "o3-mini"])
            return preferred if preferred else allowed_models[0]
